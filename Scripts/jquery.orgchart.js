...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + '>')
      ...
      .append('<div class="title" contentEditable spellcheck="false">' + nodeData[opts.nodeTitle] + '</div>')
      .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content" contentEditable spellcheck="false">' + (nodeData[opts.nodeContent] || '') + '</div>' : '');
    ...
    if (opts.draggable) {
        ...
        if (navigator.userAgent.indexOf("Firefox") > 0) event.originalEvent.dataTransfer.setData('text/html', 'hack for firefox');
        var $dragged = $(this);
  ...
  function buildParentNode($currentRoot, nodeData, opts, callback) {
    var that = this;
    var $table = $('<table style="position:relative">');
